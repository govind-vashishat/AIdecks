...
export async function CreatePowerpoint(videoId: string) {
    ...
    const { fileName, filePath } = await CreatePowerpointFromArrayOfObject(
        titleAndDescription,
        slideObjects,
        user.id
      );
...
export async function GetVideoLengthAndSubtitles(videoId: string): Promise<VideoMetaData> {
    try {
        ...
        return {
            ...
            subtitlesURL: response.data.subtitles.subtitles.find(
...
export async function CreatePowerpointFromArrayOfObject(titleAndDescription: TitleDescription, slides: SlideContent[], userId: string) {
    ...
    titleSlide.addText(titleAndDescription.title, {
      ...
      try {
        ...
        const filePath = path.join(homedir(), "/Desktop/tmp", fileName);
